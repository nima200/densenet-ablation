...
from bottleneck_featureMaps.subpixel import SubPixelUpscaling
...
def DenseNet(input_shape=None, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1, nb_layers_per_block=-1,
             ...
             classes=10, activation='softmax', growth_rate_factor=4):
...
def __conv_block(ip, nb_filter, bottleneck=False, dropout_rate=None, weight_decay=1e-4, growth_rate_factor=4):
    ...
    if bottleneck:
        inter_channel = nb_filter * growth_rate_factor  # Obtained from https://github.com/liuzhuang13/DenseNet/blob/master/densenet.lua
...
def __dense_block(x, nb_layers, nb_filter, growth_rate, bottleneck=False, dropout_rate=None, weight_decay=1e-4,
                  grow_nb_filters=True, return_concat_list=False, growth_rate_factor=4):
    ...
    for i in range(nb_layers):
        cb = __conv_block(x, growth_rate, bottleneck, dropout_rate, weight_decay, growth_rate_factor)
        x_list.append(cb)
...
def __create_dense_net(nb_classes, img_input, include_top, depth=40, nb_dense_block=3, growth_rate=12, nb_filter=-1,
                       ...
                       subsample_initial_block=False, activation='softmax', growth_rate_factor=4):
    ...
    x, nb_filter = __dense_block(x, final_nb_layer, nb_filter, growth_rate, bottleneck=bottleneck,
                                 dropout_rate=dropout_rate, weight_decay=weight_decay, growth_rate_factor=growth_rate_factor)
